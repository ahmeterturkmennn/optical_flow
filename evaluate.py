...
def create_sintel_submission(model, iters=32, warm_start=False, output_path='sintel_submission',
    ...
    for dstype in ['clean', 'final']:
        ...
        for test_id in range(len(test_dataset)):
            ...
            if warm_start:
                flow_prev = forward_interpolate(flow_low[0])[None].cpu()
...
def inference_on_video(
    ...
    save_flo_flow: bool = False,
    save_video: bool = True,                      
    video_fname: str = "video_flow.mp4",           
    start_frame: int = 0,
    stride: int = 1,
):
    model.eval()
    ...
    if not ok:
        raise RuntimeError("Video ended before start_frame.")

    flow_prev = None
    frame_idx = int(cap.get(cv2.CAP_PROP_POS_FRAMES)) - 1
    print(f"Streaming from frame {frame_idx} …")

    writer = None
    if save_video:
        writer_path = os.path.join(output_path, video_fname)
    ...
    while True:
        ...
        image1 = cv2.cvtColor(prev_bgr, cv2.COLOR_BGR2RGB)
        image2 = cv2.cvtColor(next_bgr, cv2.COLOR_BGR2RGB)
        ...
        png_file = os.path.join(output_path, base + "_flow.png")
        save_vis_flow_tofile(flow, png_file)

        if save_flo_flow:
            flo_file = os.path.join(output_path, base + "_pred.flo")

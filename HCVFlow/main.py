...
from evaluate import (validate_chairs, validate_sintel, validate_kitti,
                      ...
                      inference_on_dir,
                      inference_on_video,#added  by Ahmet Ertürkmen
...
def get_args_parser():
    ...
    #added by Ahmet Ertürkmen
    parser.add_argument('--inference_video', default=None, type=str,
                        help='Path to a video file to run flow inference on')
    parser.add_argument('--video_stride', default=1, type=int,
                        help='Process every N‑th frame pair (≥1)')
    parser.add_argument('--save_video_flow', action='store_true',
                        help='Also save an MP4 of colour‑coded flow')
...
def main(args):
    ...
    if args.inference_dir is not None:
        ...
        return
    if args.inference_video is not None:          # video inference added  by Ahmet Ertürkmen
        inference_on_video(
            model_without_ddp,
            video_path=args.inference_video,
            iters=args.val_iters,
            warm_start=args.warm_start,
            output_path=args.output_path,
            padding_factor=args.padding_factor,
            save_flo_flow=args.save_flo_flow,
            save_video=args.save_video_flow,
